...
namespace Parlot.Tests.Calc
{
    ...
    public class BiscuitTests
    {
        ...
        public async Task ParseSamples(string url)
        {
            ...
            var buffer = new BufferSpan<byte>(content);
            // System.Console.WriteLine(buffer);
            dynamic result = parsers["Biscuit"].Parse(new Fluent.Byte.ParseContext(new Scanner<byte>(buffer)));
            // System.Console.WriteLine(JsonConvert.SerializeObject(result));
            // System.Console.WriteLine(result.authority.block);
            dynamic block = parsers["Block"].Parse(new Fluent.Byte.ParseContext(new Scanner<byte>(result.authority.block)));
            var sb = new Parlot.BytesBuilder();
            Assert.True(parsers["Block"].Serializable);
            Assert.True(parsers["Block"].Serialize(sb, block));
            Assert.Equal(result.authority.block, sb.FlattenBlocks());
